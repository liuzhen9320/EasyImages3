...
**修复状态**：默认忽略 `X-Forwarded-For` 和 `Client-IP`，只使用规范化后的直连地址。新增 IPv4/IPv6 精确地址与 CIDR 可信代理配置；仅直连来源受信时从右向左解析完整转发链，并在首个非可信节点停止。上传黑白名单、游客配额、上传/登录日志、API/登录限流、分片桶、文件管理器和 TimThumb 调试记录均使用统一结果。黑白名单同时修复了显式 IP 参数和通配白名单逻辑，并支持 IPv6/CIDR。
